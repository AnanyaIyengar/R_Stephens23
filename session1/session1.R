...
practice_dta <- read_dta("R1/practice_dta.dta")
...
pit_stop <- read_fwf(file = "R1/pit_stop.txt", 
                     fwf_cols(raceID = c(1,3), driverID = c(4,5),
                              stop_no = c(6), lap_no = c(7,8),
                              milliseconds = c(9,13)))

#To import .xlsx files, we use the read_excel command from the readxl package! 

tn_sc_pop <- read_excel("R1/tn_sc_pop.xlsx")
tn_sc_edu <- read_excel("R1/tn_sc_edu.xlsx")
...
tn_sc_edu$commonid <- paste0(tn_sc_edu$dist_code, tn_sc_edu$sc_code)
tn_sc_pop$commonid <- paste0(tn_sc_pop$dist_code, tn_sc_pop$sc_code)
